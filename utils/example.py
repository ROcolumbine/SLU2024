import json
import sys
from vocab import Vocab, LabelVocab, SlotVocab
from word2vec import Word2vecUtils
...
class Example():
    ...
    def configuration(cls, root, train_path=None, word2vec_path=None):
        ...
        cls.word_vocab = Vocab(padding=True, unk=True, filepath=train_path) #从训练数据中构建的词汇表
    ...
    def load_dataset(cls, data_path, use_manual=False):
        ...
        examples = []
        for di, data in enumerate(dataset): #这里的di和下面的ui都是索引（序号）
            for ui, utt in enumerate(data):
                # print('ui= ', ui, 'utt= ', utt )
                #ex = cls(utt) # NOTE
                ex = cls(utt, f'{di}-{ui}', use_manual) # f-string，可在字符串中直接嵌入表达式
                examples.append(ex) # 插入的是一个example object
        return examples
    ...
    def __init__(self, ex: dict, did, use_manual=False):
        ...
        # # find the begining and ending index in utt for each act-slot
