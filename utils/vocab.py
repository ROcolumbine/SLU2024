...
class Vocab():
    ...
    def __init__(self, padding=False, unk=False, min_freq=1, filepath=None):
        ...
        if padding:
            ...
            self.word2id[PAD], self.id2word[idx] = idx, PAD # 添加[PAD]
        if unk:
            idx = len(self.word2id)
            self.word2id[UNK], self.id2word[idx] = idx, UNK # 添加[UNK]

        if filepath is not None:
            self.from_train(filepath, min_freq=min_freq)

    def from_train(self, filepath, min_freq=1): # 从训练数据中构建词汇表
        with open(filepath, 'r',  encoding='utf-8') as f:
            trains = json.load(f)
        word_freq = {}
        for data in trains: # 统计词频
            for utt in data:
                text = utt['manual_transcript'] #NOTE manual_transcript指转录的文本
                # text = utt['asr_1best'] # NOTE
                for char in text:
