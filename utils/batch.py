...
def from_example_list(args, ex_list, device='cpu', train=True):
    ...
    batch.utt = [ex.utt for ex in ex_list] # utt:就是用户说的话
    input_lens = [len(ex.input_idx) for ex in ex_list] # 当前batch中每个utt的长度组成的list
    max_len = max(input_lens) # 当前batch最长的utt的长度

    if args.pretrained_model:
        max_len = max([len(ex.utt) for ex in ex_list]) #这个max_len
        batch.pad_utts = batch.lm_utt = [ex.utt + ''.join([PAD]*(max_len-len(ex.utt))) for ex in ex_list] # padding使得batch内的utt等长
        batch.bert_utts = [process_utt(utt) for utt in batch.pad_utts]
    ...
    if train: #如果是训练模式
        ...
        #batch.slot_begin = torch.tensor(slot_begin, dtype=torch.long, device=device) # NOTE?
        #batch.slot_end = torch.tensor(slot_end, dtype=torch.long, device=device) # NOTE?
    else:
        batch.labels = None
        batch.tag_ids = None
